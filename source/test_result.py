...
fitData = pd.read_csv("fit_data.csv")
...
def findBestPoint(MData, machineCoefs):
    ...
    #coefsData.to_csv("coefs_data.csv", index=False)
    splitCoefsData = pd.DataFrame(coefsData['Coefs'].tolist(), columns=lamCol)
    splitCoefsData.to_csv("split_coefs_data.csv", index=True)
...
bstPntId = findBestPoint(fitData, ml.lambdas)
